...
import uetec.filesystem.server.model.Folder;
import uetec.filesystem.server.pojo.*;
import java.io.*;
...
/**
 * 
 * <h2>uetec-file-system配置解析器</h2>
 * <p>
 * 该工具负责读取并解析配置文件，并将结果随时提供给uetec-file-system服务器业务逻辑以完成相应功能（例如用户认证、权限判定、配置启动端口等）。
 * </p>
 * 
 * @author 要么出众，要么出局
 * @version 1.0
...
public class ConfigureReader {
	 ...
	 * 
	 * @author 要么出众，要么出局
	 * @return java.util.List<uetec.filesystem.server.pojo.ExtendStores> 所有扩展存储区对象的列表
	 */
	 ...
	 * <h2>获得验证码等级</h2>
	 * <p>
	 * 返回设置的验证码等级枚举类（uetec.filesystem.server.enumeration.VCLevel），包括：关闭（CLOSE）、简单（Simplified）、标准（Standard）
	 * </p>
	 * 
	 * @author 要么出众，要么出局
	 * @return uetec.filesystem.server.enumeration.VCLevel 验证码等级
	 */
			...
			if (this.testServerPropertiesAndEffect() == 0) {
				try {
					this.serverp.store(new FileOutputStream(this.confdir + SERVER_PROPERTIES_FILE),
							"<Uetec-file-system server setting file is update.>");
		...
		if ("true".equals(serverp.getProperty("https.enable"))) {
			...
			if (keyFile.isFile()) {
				...
				if (keyFile.isFile()) {
					...
					Printer.instance.print(
							"错误：无法启用https支持，因为uetec-file-system未能找到https证书文件。您必须在应用主目录内放置PKCS12（必须命名为https.p12）或JKS（必须命名为https.jks）证书。");
	...
	public void createDefaultServerPropertiesFile() {
		...
		try {
			dsp.store(new FileOutputStream(this.confdir + SERVER_PROPERTIES_FILE),
					"<This is the default uetec-file-system server setting file. >");
	...
	private void createDefaultAccountPropertiesFile() {
		...
		try {
			dap.store(new FileOutputStream(this.confdir + ACCOUNT_PROPERTIES_FILE),
					"<This is the default uetec-file-system account setting file. >");
		...
		if (accountPropertiesUpdateDaemonThread == null) {
			...
			accountPropertiesUpdateDaemonThread = new Thread(() -> {
				try {
					...
					// TODO 自动生成的 catch 块
					Printer.instance.print("错误：用户配置文件更改监听失败，该功能已失效，uetec-file-system可能无法实时更新用户配置（重启应用可恢复该功能）。");
				}
			});
