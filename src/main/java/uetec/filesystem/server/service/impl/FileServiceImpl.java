...
import uetec.filesystem.server.pojo.CheckImportFolderRespons;
import uetec.filesystem.server.pojo.CheckUploadFilesRespons;
 ...
 * @author 要么出众，要么出局
 * @version 1.0
 * @see uetec.filesystem.server.service.FileService
 */
@Service
public class FileServiceImpl extends RangeFileStreamWriter implements FileService {
	...
	public void downloadCheckedFilesZip(final HttpServletRequest request, final HttpServletResponse response)
		...
		if (zipname != null && !zipname.equals("ERROR")) {
			...
			String fname = "uetec-file-system_" + ServerTimeUtil.accurateToDay() + "_\u6253\u5305\u4e0b\u8f7d.zip";
			if (zip.exists()) {
