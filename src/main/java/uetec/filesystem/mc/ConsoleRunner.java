...
import uetec.filesystem.server.model.Node;
...
import uetec.filesystem.util.file_system_manager.FileSystemManager;
import uetec.filesystem.util.file_system_manager.pojo.Folder;
import uetec.filesystem.util.file_system_manager.pojo.FolderView;

/**
 * 
 * <h2>命令模式启动器</h2>
 * <p>
 * 该启动器将以命令模式启动uetec-file-system，请执行静态build()方法开启界面并初始化uetec-file-system服务器引擎。
 * </p>
 * 
 * @author 要么出众，要么出局
 * @version 1.0
...
public class ConsoleRunner {
	 ...
	 * @return uetec.filesystem.mc.ConsoleRunner 本启动器唯一实例
	 */
	...
	// 执行相应的指令并进行后续处理，该方法为整个uetec-file-system命令模式的起点。
	private void execute(final String[] args) {
		if (args.length > 0) {
			...
			default: {
				Printer.instance
						.print("uetec-file-system:无效的指令，使用控制台模式启动请输入参数 -console，备份文件请输入参数 -backup {本地备份路径}，使用UI模式启动请不传入任何参数。");
				break;
			}
			}
		}
	...
	private void startKiftdByConsole() {
		Printer.instance.print(" 连营电子网络文件系统 控制台模式[Console model]");
	...
	private void startServer() {
		...
		if (ConsoleRunner.ctl.started()) {
			...
			Printer.instance.print("服务器已启动，可以正常访问了，您可以使用 -status 指令查看运行状态。");
		} else {
			if (ConfigureReader.instance().getPropertiesStatus() != 0) {
				switch (ConfigureReader.instance().getPropertiesStatus()) {
				case ConfigureReader.INVALID_PORT:
					Printer.instance.print("错误：服务器未能启动，端口设置无效。");
					break;
				case ConfigureReader.INVALID_BUFFER_SIZE:
					Printer.instance.print("错误：服务器未能启动，缓存设置无效。");
					break;
				case ConfigureReader.INVALID_FILE_SYSTEM_PATH:
					Printer.instance.print("错误：服务器未能启动，文件系统路径或某一扩展存储区设置无效。");
					break;
				case ConfigureReader.INVALID_LOG:
					Printer.instance.print("错误：服务器未能启动，日志设置无效。");
					break;
				case ConfigureReader.INVALID_VC:
					Printer.instance.print("错误：服务器未能启动，登录验证码设置无效。");
					break;
				default:
					Printer.instance.print("错误：服务器未能启动，请重试或检查设置。");
					break;
				}
			} else {
				Printer.instance.print("错误：服务器未能启动，请重试或检查设置。");
	...
	private void awaiting() {
		Thread t = new Thread(() -> {
			...
			try {
				while (true) {
					System.out.print("uetec-file-system: console$ ");
				...
				Printer.instance.print("错误：读取命令时出现意外导致程序退出，请重启服务器。");
			}
		});
		t.start();
	}
	...
	private void fileSystemManagerModel() {
		...
		try {
			...
			Printer.instance.print("错误：无法打开文件系统，该文件系统可能正在被另一个文件系统程序占用。");
			return;
		}
		System.out.println("命令帮助：\r\n" + fsCommandTips + "\r\n");
		try {
			while (true) {
				System.out.print("uetec-file-system: " + currentFolder.getCurrent().getFolderName() + "$ ");
				String command = new String(reader.nextLine().getBytes("UTF-8"), "UTF-8");
				// 针对一些带参数指令的操作
		...
		while (true) {
			...
			case "Y":
				try {
					...
					Printer.instance.print("错误：无法读取根目录，请尝试重新打开文件管理系统或重启服务。");
				}
				return;
			case "N":
