package uetec.filesystem.ui.module;
...
public class ServerUIModule extends KiftdDynamicWindow {
	...
	private ServerUIModule() {
		setUIFont();
		(ServerUIModule.window = new JFrame("连营电子网络文件系统控制台")).setSize(OriginSize_Width, OriginSize_Height);
		ServerUIModule.window.setLocation(100, 100);
		ServerUIModule.window.setResizable(false);
		try {
			ServerUIModule.window.setIconImage(
					ImageIO.read(this.getClass().getResourceAsStream("/uetec/filesystem/ui/resource/icon.png")));
		...
		if (SystemTray.isSupported()) {
			...
			String iconType = "/uetec/filesystem/ui/resource/icon_tray.png";
			if (System.getProperty("os.name").toLowerCase().indexOf("window") >= 0) {
				iconType = "/uetec/filesystem/ui/resource/icon_tray_w.png";
			}
			try {
				(ServerUIModule.trayIcon = new TrayIcon(ImageIO.read(this.getClass().getResourceAsStream(iconType))))
						.setToolTip("连营电子网络文件系统");
		...
		final JLabel title = new JLabel("uetec-file-system");
		...
		final JLabel subtitle = new JLabel("连营电子网络文件系统-服务器");
		...
		bottombox.add(new JLabel("--yukai.guo@uetec.cn--"));
					...
					Thread t = new Thread(() -> {
						if (ss.start()) {
							...
							if (st.getServerStatus()) {
								printMessage("服务器已经启动，可以正常访问了。");
							} else {
								printMessage("服务器未能成功启动，请检查设置或查看异常信息。");
							}
						} else {
							if(ConfigureReader.instance().getPropertiesStatus() != 0) {
								...
								case ConfigureReader.INVALID_PORT:
									printMessage("服务器无法启动：端口设置无效。");
									break;
								case ConfigureReader.INVALID_BUFFER_SIZE:
									printMessage("服务器无法启动：缓存设置无效。");
									break;
								case ConfigureReader.INVALID_FILE_SYSTEM_PATH:
									printMessage("服务器无法启动：文件系统路径或某一扩展存储区设置无效。");
									break;
								case ConfigureReader.INVALID_LOG:
									printMessage("服务器无法启动：日志设置无效。");
									break;
								case ConfigureReader.INVALID_VC:
									printMessage("服务器无法启动：登录验证码设置无效。");
									break;
								default:
									printMessage("服务器无法启动，请检查设置或查看异常信息。");
									break;
								}
							}else {
								printMessage("服务器无法启动，请检查设置或查看异常信息。");
				...
				Thread t = new Thread(() -> {
					if (cs != null) {
						...
						if (cs.close()) {
							...
							if (st.getServerStatus()) {
								printMessage("服务器未能成功关闭，如有需要，可以强制关闭程序（不安全）。");
							} else {
								printMessage("服务器已经关闭，停止所有访问。");
							}
						} else {
							printMessage("服务器无法关闭，请手动结束本程序。");
						}
						updateServerStatus();
					}
				});
