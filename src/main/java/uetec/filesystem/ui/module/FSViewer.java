...
import uetec.filesystem.ui.util.FilesTable;
...
public class FSViewer extends KiftdDynamicWindow {
	...
	private FSViewer() throws SQLException {
		...
		window = new JDialog(ServerUIModule.window, "uetec-file-system-ROOT");
	...
	public void show() {
		...
		try {
			...
			// TODO 自动生成的 catch 块
			Printer.instance.print("错误：无法打开文件系统，该文件系统可能正在被另一个uetec-file-system占用。");
		}
	}
	...
	private void getFolderView(String folderId) throws Exception {
		try {
			...
			if (currentView != null && currentView.getCurrent() != null) {
				...
				window.setTitle("uetec-file-system-" + currentView.getCurrent().getFolderName());
			} else {
				// 浏览一个不存在的文件夹时自动返回根目录
				getFolderView("root");
			}
	 ...
	 * 
	 * @author 要么出众，要么出局
	 * @return uetec.filesystem.ui.module.FSViewer 该视图的唯一实例，程序中只会存在一个该窗口。
	 */
